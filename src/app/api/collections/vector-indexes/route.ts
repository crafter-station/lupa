...
import { VECTOR_INDEX_TABLE } from "@/db";
...
export async function GET(request: Request) {
  ...
  originUrl.searchParams.set("table", VECTOR_INDEX_TABLE);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const data = schema.VectorIndexInsertSchema.parse(json);
    ...
    const result = await db.transaction(async (tx) => {
      await tx.insert(schema.VectorIndex).values(data);
