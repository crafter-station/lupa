...
export async function GET(request: Request) {
  ...
  originUrl.searchParams.set("table", BUCKET_TABLE);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { id, name, description } = schema.BucketInsertSchema.parse(json);
    ...
    const result = await db.transaction(async (tx) => {
      await tx.insert(schema.Bucket).values({
        id,
        name,
        description,
      });
