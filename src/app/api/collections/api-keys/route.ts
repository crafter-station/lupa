...
export async function GET(request: Request) {
  ...
  originUrl.searchParams.set("table", API_KEY_TABLE);

  const session = await auth();
  const projectId = url.searchParams.get("params[project_id]");

  if (!projectId) {
    return Response.json(
      { success: false, error: "project_id parameter required" },
      { status: 400 },
    );
  }
  ...
  if (whereValue) {
    originUrl.searchParams.set(
      "where",
      `project_id = '${projectId}' AND ${whereValue}`,
    );
  } else {
    originUrl.searchParams.set("where", `project_id = '${projectId}'`);
  }
